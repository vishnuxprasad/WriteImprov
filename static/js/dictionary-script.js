      ...
      for (let j = 0; j < meanings.length; j++) {
        ...
        for (let k = 0; k < definitions.length; k++) {
          ...
          if (!foundDefinition && definition.definition) {
            ...
            document.querySelector(".word span").innerText = `${partOfSpeech} ${result[i].phonetics[0]?.text || ''}`;
        ...
        if (foundDefinition) {
          ...
          if (synonymsList.length > 0) {
            ...
            for (let l = 0; l < synonymsList.length; l++) {
              ...
              const tag = `<span onclick="search('${synonym}')">${synonym}</span>${l !== synonymsList.length - 1 ? ',' : ''}`;
              synonyms.insertAdjacentHTML("beforeend", tag);
              if (l === 4) break; // Display only the first 5 synonyms
            }
          } else {
            synonyms.parentElement.style.display = "none";
          }
  
          // Handle audio and speaker icon
          ...
          return; // Exit the loop after displaying the first available definition, example, and synonyms
        }
      }
    }
}
...
function fetchApi(word){
    word = word.trim(); // Trim the word variable

    if (word === "") {
      // If the input is empty, show the "Please enter a word and hit search" message
